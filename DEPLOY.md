# Deploying BillWatch to Amazon Bedrock AgentCore

## 0. Prerequisites
- An AWS account with Amazon Bedrock model access enabled for the model in
  `billwatch/config.py` (`MODEL_ID`). In us-east-1 the default cross-region
...
## 6. Tear down
```bash
make destroy     # agentcore remove agent --name billwatch --yes || true; agentcore deploy --yes
```

Repo: https://github.com/nareshsaladi54-wq/billwatch-agent
